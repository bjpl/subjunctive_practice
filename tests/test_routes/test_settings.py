...
Tests the database-backed user settings API including:
...
from api.routes.settings import UserSettings
...
@pytest.fixture
def settings_test_user(db_session):
    """Create a user specifically for settings tests."""
    user = User(
...
@pytest.fixture
def client(override_get_db, override_settings, settings_test_user):
    """Create test client with database and security overrides."""
    from core.security import get_current_user, get_current_active_user

    async def override_get_current_user():
        return {
            "sub": settings_test_user.username,
            "username": settings_test_user.username,
            "email": settings_test_user.email,
            "type": "access"
        }
...
@pytest.fixture
def auth_headers(settings_test_user, test_settings):
    """Create auth headers with a valid token for the settings test user."""
    token_data = {
        "sub": settings_test_user.username,
        "username": settings_test_user.username,
        "email": settings_test_user.email,
...
class TestGetSettings:
    ...
    def test_get_settings_unauthenticated(self, override_get_db, override_settings):
        """Test that unauthenticated requests are rejected."""
        # Use a fresh client without auth overrides
        unauthenticated_client = TestClient(app)
        response = unauthenticated_client.get("/api/settings")
