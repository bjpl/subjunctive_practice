...
def parse_user_id(user_sub: str) -> int:
    """
    Parse user ID from JWT subject field.

    Handles formats like 'user_7' or '7'.

    Args:
        user_sub: User subject from JWT token

    Returns:
        Integer user ID

    Raises:
        HTTPException: If user_sub format is invalid
    """
    try:
        if isinstance(user_sub, str):
            # Extract numeric part if user_id is like "user_7"
            user_id = int(user_sub.split('_')[-1]) if '_' in user_sub else int(user_sub)
        else:
            user_id = int(user_sub)
        return user_id
    except (ValueError, IndexError):
        logger.error(f"Invalid user_id format: {user_sub}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid user identifier format: {user_sub}"
        )
